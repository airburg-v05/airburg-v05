...
interface TmallTargetDiagnosticsProps {
  ...
  return (
    <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-4 text-sm leading-6 text-amber-800">
      <p className="font-semibold">
        本地目标数据不可用，暂无法生成目标诊断。请前往目标管理页面检查。
      </p>
  ...
  return (
    <div className="flex min-h-[180px] flex-col items-center justify-center rounded-xl border border-dashed border-slate-200 bg-slate-50 px-4 py-8 text-center">
      <p className="text-sm font-semibold text-slate-900">
        暂无需要展示的目标诊断。请先到目标管理页面创建目标，或等待更多经营数据后再查看。
      </p>
...
function DiagnosticItem({ item }: { item: TmallTargetDiagnosticItem }) {
  return (
...
export function TmallTargetDiagnostics({
  summary,
  targetStorageStatus,
  seriesStorageCorrupted,
}: TmallTargetDiagnosticsProps) {
  return (
    <SectionCard
      title="目标诊断提示"
      description="基于当前目标完成情况生成规则化提示，帮助快速识别需要关注的目标。诊断只基于已上传数据，不做 AI 预测。"
      action={
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-xl border border-slate-100 bg-slate-50 p-4">
            <div className="flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between">
              <div>
                <p className="text-sm font-semibold text-slate-900">优先处理提示</p>
                <p className="mt-1 text-xs text-slate-500">
                  首页最多展示 5 条，完整目标创建、编辑和暂停请进入目标管理。
                </p>
              </div>
