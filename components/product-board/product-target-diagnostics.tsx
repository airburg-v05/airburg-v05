...
interface ProductTargetDiagnosticsProps {
  ...
  return (
    <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-4 text-sm leading-6 text-amber-800">
      <p className="font-semibold">
        本地目标数据不可用，暂无法生成当前商品目标诊断。请前往目标管理页面检查。
      </p>
...
function EmptyProductDiagnosticsState() {
  return (
    <div className="flex min-h-[180px] flex-col items-center justify-center rounded-xl border border-dashed border-slate-200 bg-slate-50 px-4 py-8 text-center">
      <p className="text-sm font-semibold text-slate-900">
        暂无需要展示的当前商品目标诊断。请先到目标管理页面创建宝贝目标，或等待更多经营数据后再查看。
      </p>
...
function ProductDiagnosticItem({ item }: { item: TmallTargetDiagnosticItem }) {
  return (
    <li className="rounded-xl border border-slate-100 bg-white px-4 py-3">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-xs leading-5 text-slate-500">{productSuggestion(item.suggestion)}</p>
        </div>
...
const productNotice = (notice: string): string =>
  notice === "宝贝和系列推广目标只使用商品推广报表，不使用计划推广报表补齐。"
    ? "宝贝推广目标只使用商品推广报表，不使用计划推广报表补齐。"
    : notice;

const productSuggestion = (suggestion: string): string =>
  suggestion.replace("该宝贝或系列商品", "该宝贝");

export function ProductTargetDiagnostics({
  summary,
  targetStorageStatus,
}: ProductTargetDiagnosticsProps) {
  return (
    <SectionCard
      title="当前商品目标诊断提示"
      description="基于当前商品目标完成情况生成规则化提示，帮助快速识别该商品经营和推广目标的关注点。诊断只基于已上传数据，不做 AI 预测。"
      action={
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-xl border border-slate-100 bg-slate-50 p-4">
            <div className="flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between">
              <div>
                <p className="text-sm font-semibold text-slate-900">当前商品目标提示</p>
                <p className="mt-1 text-xs text-slate-500">
                  宝贝看板最多展示 5 条，仅包含当前商品目标；创建、编辑和暂停请进入目标管理。
                </p>
              </div>
            ...
            {summary.items.length > 0 ? (
              <ul className="mt-4 grid gap-3">
                {summary.items.map((item) => (
                  <ProductDiagnosticItem key={item.id} item={item} />
                ))}
              </ul>
            ) : (
          ...
          <div className="rounded-xl border border-slate-100 bg-white px-4 py-3 text-sm leading-6 text-slate-600">
            <div className="space-y-2">
              {summary.notices.map((notice) => (
                <p key={notice}>{productNotice(notice)}</p>
              ))}
            </div>
          </div>
        </div>
