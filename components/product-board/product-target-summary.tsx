...
import type {
  ...
  TmallProductTargetSummaryCard,
  TmallProductTargetSummaryViewModel,
} from "@/lib/tmall/view-models/product-target-summary";

interface ProductTargetSummaryProps {
  summary: TmallProductTargetSummaryViewModel | null;
  targetStorageStatus: TmallTargetStorageStatus;
}

const toneClasses: Record<TmallProductTargetSummaryCard["tone"], string> = {
...
const displayCardValue = (value: TmallProductTargetSummaryCard["value"]): string => {
  if (value === null) return "--";
  if (typeof value === "number") return Number.isFinite(value) ? String(value) : "--";
  return value.trim() ? value : "--";
};

const displayGap = (item: TmallProductTargetItem): string => {
...
function EmptyProductTargetState() {
  return (
...
function ProductTargetItemRow({ item }: { item: TmallProductTargetItem }) {
  return (
...
export function ProductTargetSummary({
  summary,
  targetStorageStatus,
}: ProductTargetSummaryProps) {
  return (
    <SectionCard
      title="当前商品目标完成情况"
      description="基于当前商品目标和四源安全聚合结果，查看该商品经营与推广目标完成情况。目标创建和编辑请进入目标管理页面。"
      action={
    ...
    >
      ...
      {targetStorageStatus !== "corrupted" && summary?.totalProductTargetCount === 0 ? (
        <EmptyProductTargetState />
      ) : null}

      {targetStorageStatus !== "corrupted" && summary && summary.totalProductTargetCount > 0 ? (
        <div className="space-y-4">
          ...
          <div className="flex flex-wrap gap-2 rounded-xl border border-slate-100 bg-slate-50 p-4 text-xs font-semibold text-slate-600">
            <span>启用 {summary.activeProductTargetCount} 个</span>
            <span>暂停 {summary.pausedProductTargetCount} 个</span>
          ...
          <div className="rounded-xl border border-slate-100 bg-slate-50 p-4">
            <div className="flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between">
              <div>
                <p className="text-sm font-semibold text-slate-900">当前商品目标列表</p>
                <p className="mt-1 text-xs text-slate-500">
                  仅展示当前商品目标，最多 6 条；目标创建、编辑、暂停和删除请进入目标管理。
                </p>
              </div>
            ...
            {summary.targetItems.length > 0 ? (
              <ul className="mt-4 grid gap-3">
                {summary.targetItems.map((item) => (
                  <ProductTargetItemRow key={item.targetId} item={item} />
                ))}
              </ul>
            ) : (
