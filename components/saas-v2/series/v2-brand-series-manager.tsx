        ...
        <div className="flex min-w-0 flex-1 items-center gap-3">
          ...
          {visibleSeries.length > 0 ? (
            <label className="flex min-w-0 items-center gap-1.5 text-[11px] font-semibold text-slate-500">
              当前
              <select
                className="h-8 min-w-0 max-w-52 rounded-md border border-slate-200 bg-white px-2 text-xs font-semibold text-slate-700"
                onChange={(event) => onSelectSeries(event.target.value)}
                value={selectedSeriesId ?? visibleSeries[0]?.seriesId ?? ""}
              >
                {visibleSeries.map((record) => <option key={record.seriesId} value={record.seriesId}>{record.name}</option>)}
              </select>
            </label>
          ) : null}
        </div>
