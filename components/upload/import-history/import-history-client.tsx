...
function ImportHistoryTable({
  ...
  onSelectEntry: (entry: ImportHistoryEntry) => void;
}) {
  if (entries.length === 0) {
    return (
      ...
      <aside className="relative flex h-full w-full flex-col bg-white shadow-xl sm:max-w-[560px]">
        <div className="flex items-start justify-between gap-4 border-b border-slate-100 px-5 py-4">
          <div className="min-w-0">
            ...
            <h2 className="mt-1 text-lg font-semibold text-slate-950">{entry.storeName}</h2>
...
export function ImportHistoryClient() {
  const [filters, setFilters] = useState<ImportHistoryFilters>(DEFAULT_IMPORT_HISTORY_FILTERS);
  ...
  if (loading) {
    return <SafeStateCard title="正在读取导入记录" description="正在通过 V2 公开读取接口加载本地历史。" actionLabel="返回数据导入" />;
  }

  if (result.status === "empty") {
    return <SafeStateCard title="暂无导入记录" description="当前还没有 V2 导入记录。请先返回数据导入页完成一次批量导入。" />;
  }

  if (result.status === "corrupted") {
    return <SafeStateCard title="历史数据不可安全读取" description="本地 V2 历史数据可能损坏。页面不会自动删除数据，请重新上传并导入修复。" />;
  }
      ...
      <SectionCard
        ...
        action={
          <div className="text-xs text-slate-400">
            Active dataset：<span className="font-mono" title={activeDatasetId ?? ""}>{shortId(activeDatasetId)}</span>
          </div>
        }
      >
        ...
        <ImportHistoryTable entries={entries} onSelectEntry={setSelectedEntry} />
      </SectionCard>

      <ImportHistoryDrawer entry={selectedEntry} onClose={() => setSelectedEntry(null)} />
    </div>
  );
}
