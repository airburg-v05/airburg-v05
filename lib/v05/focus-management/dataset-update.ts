...
import type { FocusDatasetMutation, FocusSaveResult } from "./contracts";

export const cloneFocusValue = <T>(value: T): T => JSON.parse(JSON.stringify(value)) as T;

const success = (message: string, datasetId: string | null): FocusSaveResult => ({
  status: "success",
  message,
  datasetId,
  issueCodes: [],
...
const withDeterministicDatasetId = async (dataset: V2StagingDataset): Promise<V2StagingDataset> => {
  ...
  return {
    ...dataset,
    datasetId: `v05e1_focus_dataset_${hash.slice(0, 32)}`,
  };
};
...
export const buildPreparedFocusDataset = async ({
  dataset,
  currentMetadata,
  preparedAt,
}: {
  ...
  preparedAt: string;
}): Promise<PreparedV2Dataset | FocusSaveResult> => {
  const manifest = dataset.migrationManifests[0];
  if (!manifest) {
    return focusError("validation_error", "当前数据缺少可追踪的导入记录，暂不能保存管理对象。", [
...
export const saveFocusDatasetMutation = async ({
  store,
  ...
  if (!currentDatasetId) return focusError("empty", "当前没有可保存的 active 数据，请先完成数据导入。");
  if (currentDatasetId !== expectedCurrentDatasetId) {
    return focusError("conflict", "当前数据已被其他操作更新，请刷新后重试。", ["active_dataset_conflict"]);
  }

  const activeDataset = await store.loadDataset(currentDatasetId);
  if (!activeDataset) return focusError("empty", "当前 active 数据不可读取，请先检查数据质量。");
  const currentMetadata = await store.getDatasetMetadata(currentDatasetId);

  const workingDataset = cloneFocusValue(activeDataset);
  ...
  const prepared = await buildPreparedFocusDataset({
    dataset: result,
    currentMetadata,
    preparedAt: now,
  });
  ...
  if (write.status !== "prepared") {
    return focusError("error", "写入本地 staging 失败，未激活新数据。", write.issues.map((issue) => issue.code));
  }
  ...
  if (readBack.status !== "readback_validated") {
    return focusError("error", "本地 readback 校验失败，未激活新数据。", readBack.issues.map((issue) => issue.code));
  }
  ...
  if (activation.status === "conflict") {
    return focusError("conflict", "当前数据已被其他操作更新，请刷新后重试。", activation.issues.map((issue) => issue.code));
  }
  if (activation.status !== "activated" && activation.status !== "already_active") {
    return focusError("error", "激活新数据失败，当前 active 数据未改变。", activation.issues.map((issue) => issue.code));
  }
