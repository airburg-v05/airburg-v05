...
const main = async () => {
  ...
  addCheck(
    "brandPopoverExplainsSafeDebugBoundary",
  ...
  addCheck(
    "tempProductOverridesDataOptionDisplay",
    product.includes("const existing = options.get(key)") &&
      product.includes("...(existing ?? product)") &&
      product.includes("source: \"temp\""),
  );
