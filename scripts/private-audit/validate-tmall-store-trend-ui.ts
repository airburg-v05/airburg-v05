...
import type { TmallStoreTrendCardViewModel } from "../../lib/tmall/view-models/store-trend-section";
...
): TmallStoreTrendCardViewModel | null =>
  cards.find((card) => card.metricKey === metricKey) ?? null;

const latestValuesAreSafe = (cards: TmallStoreTrendCardViewModel[]): boolean =>
  cards.every((card) => card.latestValue === null || Number.isFinite(card.latestValue));

const changeRatesAreSafe = (cards: TmallStoreTrendCardViewModel[]): boolean =>
...
const main = async () => {
  ...
  const section = buildTmallStoreTrendSection(result);
  ...
  const safeLeafValues = collectLeafValues(section);
  ...
  const containsSensitiveFieldName = SENSITIVE_FIELD_NAMES.some((fieldName) =>
    JSON.stringify(section).includes(fieldName),
  );

  const summary = {
    cardCount: section.cards.length,
    storeGmvPointCount: gmvCard?.pointCount ?? 0,
    storeGmvInsufficient: gmvCard?.series.insufficientDataForTrend ?? false,
    adSpendPointCount: adSpendCard?.pointCount ?? 0,
    adSpendInsufficient: adSpendCard?.series.insufficientDataForTrend ?? true,
    adRoiPointCount: adRoiCard?.pointCount ?? 0,
    adRoiInsufficient: adRoiCard?.series.insufficientDataForTrend ?? true,
    hasInvalidNumber:
      hasInvalidNumber(section) ||
  ...
  const checksPassed =
    summary.cardCount === 6 &&
    summary.storeGmvPointCount === 1 &&
    summary.storeGmvInsufficient &&
    summary.adSpendPointCount === 7 &&
    !summary.adSpendInsufficient &&
    summary.adRoiPointCount === 7 &&
    !summary.adRoiInsufficient &&
...
main().catch((error: unknown) => {
  console.error(error instanceof Error ? error.message : "store_trend_ui_validation_failed");
  process.exitCode = 1;
});
