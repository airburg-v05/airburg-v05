...
const launchChrome = async (baseUrl: string): Promise<{ client: CdpClient; process: ChildProcess; profileDir: string }> => {
  ...
  const profileDir = fs.mkdtempSync(path.join(os.tmpdir(), "airburg-v05g2-mobile-chrome-"));
  const child = spawn(
    chromePath,
    [
      "--headless=new",
...
const setDemoSession = async (client: CdpClient): Promise<void> => {
  await evaluate<void>(
    client,
    `localStorage.setItem("airburg:demo-session", JSON.stringify({account:"g2-mobile-audit@airburg.local",loggedInAt:"2026-06-23T00:00:00.000Z"}));`,
  );
};
...
const main = async () => {
  ...
  const screenshotDir = fs.mkdtempSync(path.join(os.tmpdir(), "airburg-v05g2-screenshots-"));
  ...
  try {
    ...
    const removedScreenshotFiles = screenshots.every((record) => {
      const filePath = path.join(screenshotDir, `${record.key}.png`);
    ...
    const checks = {
      ...
      screenshotFilesCleaned: removedScreenshotFiles,
