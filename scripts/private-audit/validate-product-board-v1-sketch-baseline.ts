...
const BASE_URL = process.env.PRODUCT_BOARD_V1_AUDIT_BASE_URL ?? "http://localhost:3000";
const PAGE_URL = `${BASE_URL.replace(/\/$/, "")}/product-board`;
const CHROME_PATH = "/Applications/Google Chrome.app/Contents/MacOS/Google Chrome";
const screenshotDir = fs.mkdtempSync(path.join(os.tmpdir(), "airburg-product-board-v1-"));
...
const clickKpi = async (client: CdpClient, title: string) => {
  ...
  await evaluate<void>(
    client,
    `(() => {
      const target = Array.from(document.querySelectorAll('[data-testid="product-board-v1-kpi-card"]'))
...
const main = async () => {
  ...
  const component = read("components/product-board/v1/product-board-v1-dashboard.tsx");
  const visualSystemSource = read("components/visual-system/v1/visual-system.tsx");
  const visualSources = `${component}\n${visualSystemSource}`;
  const visibleTextSource = component.replaceAll('typeof window === "undefined"', "");
  ...
  const staticChecks = {
    pageUsesV1: page.includes("ProductBoardV1Dashboard"),
    navItemsExist: navItems.every((item) => visualSources.includes(item)),
    productBoardHighlighted: component.includes('activeLabel="宝贝看板"'),
    topbarExists: component.includes('title="宝贝看板"') && visualSystemSource.includes("退出"),
    controlsExist: ["目标店铺", "宝贝设置", "统计时间", "自定义"].every((text) => component.includes(text)),
    noHomeTerms:
    ...
    productTopExists: component.includes("单品流量漏斗"),
    omittedBigModules:
      !component.includes("推广表现") &&
      !component.includes("所属系列") &&
      !component.includes("售后风险"),
    noInvalidSourceText:
      !['"NaN"', "'NaN'", "`NaN`", '"Infinity"', "'Infinity'", "`Infinity`", '"undefined"', "'undefined'", "`undefined`"].some(
        (token) => visibleTextSource.includes(token),
      ),
  ...
  const profileDir = fs.mkdtempSync(path.join(os.tmpdir(), "airburg-product-board-v1-profile-"));
  ...
  try {
    ...
    await client.send("Page.addScriptToEvaluateOnNewDocument", {
      source: `window.localStorage.setItem("airburg:demo-session", JSON.stringify({ account: "product-v1-audit", loggedInAt: "2026-06-27T00:00:00.000Z" }));`,
    });
    ...
    screenshots.push(await capture(client, "product-board-1440", "1440x1200", "base"));

    await setViewport(client, 390, 1000);
    screenshots.push(await capture(client, "product-board-390", "390x1000", "base"));

    await setViewport(client, 1440, 1200);
    await clickByText(client, "宝贝设置");
    await waitForSelector(client, '[data-testid="product-board-v1-product-settings-popover"]');
    screenshots.push(await capture(client, "product-board-product-settings", "1440x1200", "product_settings_open"));
    ...
    screenshots.push(await capture(client, "product-board-return-rate-three-lines", "1440x1200", "return_rate_click"));

    await evaluate<void>(
      client,
      `document.querySelector('[data-testid="product-board-v1-product-top"]')?.scrollIntoView({ block: 'center' });`,
    );
    await wait(300);
    screenshots.push(await capture(client, "product-board-product-top", "1440x1200", "product_top"));
  } finally {
    if (client) {
  ...
  fs.writeFileSync(
    manifestPath,
    JSON.stringify({ route: "/product-board", screenshotDir, screenshots, checks, forbiddenChanged, consoleErrors: capturedConsoleErrors, ignoredConsoleErrors }, null, 2),
    "utf8",
  );
