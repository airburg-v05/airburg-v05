...
const DATABASE_NAME = "airburg-v05-d1-audit";
...
const launchChrome = async (chromePath: string, baseUrl: string): Promise<{
  ...
  const userDataDir = fs.mkdtempSync(path.join(os.tmpdir(), "airburg-v05d1-chrome-"));
...
const staticDependencyChecks = (): { c31CompletionValid: boolean } => {
  const completion = JSON.parse(readText(C31_COMPLETION)) as {
    taskId?: string;
    status?: string;
  ...
  return {
    c31CompletionValid:
      completion.taskId === "V0.5C_3_1_STAGE_SCOPED_FREEZE_GATE_AND_POST_REGISTRATION_CLOSURE" &&
...
const runRuntime = async (sensitiveValues: Set<string>): Promise<RuntimeEvidence> => {
  ...
  const screenshotDir = fs.mkdtempSync(path.join(os.tmpdir(), "airburg-v05d1-store-screenshots-"));
  ...
  try {
    ...
    const defaultStoreBoardWorks = await client.evaluate<boolean>(`
      ...
      document.body.innerText.includes("商品 TOP5") &&
      document.querySelectorAll('svg[aria-label="店铺主趋势图"]').length === 1
    `);
    ...
    fs.writeFileSync(completedScreenshots.manifestPath, JSON.stringify(completedScreenshots, null, 2));
...
const main = async () => {
  ...
  const screenshotPaths = runtime?.screenshots
    ...
    : [];
  const checks = {
    c31CompletionValid: staticChecks.c31CompletionValid,
    ...
    screenshotsExist: screenshotPaths.length >= 9 && screenshotPaths.every((filePath) => fs.existsSync(filePath)),
  ...
  const output = {
    ...
    script: "validate-v05d1-store-board-ui",
