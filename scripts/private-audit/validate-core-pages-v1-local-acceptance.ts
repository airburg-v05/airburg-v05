...
const BASE_URL = process.env.CORE_PAGES_V1_AUDIT_BASE_URL ?? "http://localhost:3000";
const CHROME_PATH = "/Applications/Google Chrome.app/Contents/MacOS/Google Chrome";
const screenshotDir = fs.mkdtempSync(path.join(os.tmpdir(), "airburg-core-pages-v1-"));
...
const pageSpecs: PageSpec[] = [
  {
    ...
    minCardCount: 17,
    sourceFiles: ["components/home/home-bi-dashboard.tsx", "lib/bi/bi.home-mapper.ts"],
    requiredTexts: [
      "目标店铺",
      "商品排除",
      "平台目标",
  ...
  },
  {
    ...
    sourceFiles: ["components/series-board/v1/series-board-v1-dashboard.tsx"],
    requiredTexts: ["目标店铺", "系列设置", "商家备注", "系列目标", "系列GMV", "直接成交占比", "系列内商品贡献 TOP"],
  ...
  },
  {
    ...
    expectedCardCount: 15,
    sourceFiles: ["components/store-board/v1/store-board-v1-dashboard.tsx"],
    requiredTexts: ["目标店铺", "店铺设置", "店铺GMV", "直接成交占比", "店铺内商品贡献 TOP"],
  ...
  },
  {
    ...
    expectedCardCount: 15,
    sourceFiles: ["components/product-board/v1/product-board-v1-dashboard.tsx"],
    requiredTexts: ["目标店铺", "宝贝设置", "宝贝GMV", "直接成交占比", "单品流量漏斗"],
  ...
  },
  {
    ...
    sourceFiles: ["components/upload/v1/upload-page-v1-dashboard.tsx"],
    requiredTexts: [
      "目标店铺",
      "批量导入经营数据",
      "生意参谋商品经营表",
  ...
  },
  {
    ...
    sourceFiles: ["components/upload/history/v1/history-data-v1-dashboard.tsx"],
    requiredTexts: [
      "平台筛选",
      "店铺筛选",
      "开始日期",
...
const runBrowserAudit = async (): Promise<ScreenshotRecord[]> => {
  const port = 9674;
  const profileDir = fs.mkdtempSync(path.join(os.tmpdir(), "airburg-core-pages-v1-profile-"));
  ...
  try {
    ...
    await evaluate<void>(
      client,
      `localStorage.setItem("airburg:demo-session", JSON.stringify({ account: "core-pages-v1-audit", loggedInAt: "2026-06-27T00:00:00.000Z" }))`,
    );
