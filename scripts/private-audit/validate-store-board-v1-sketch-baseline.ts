...
const BASE_URL = process.env.STORE_BOARD_V1_AUDIT_BASE_URL ?? "http://localhost:3000";
const PAGE_URL = `${BASE_URL.replace(/\/$/, "")}/store-board`;
const CHROME_PATH = "/Applications/Google Chrome.app/Contents/MacOS/Google Chrome";
const screenshotDir = fs.mkdtempSync(path.join(os.tmpdir(), "airburg-store-board-v1-"));
...
const clickKpi = async (client: CdpClient, title: string) => {
  ...
  await evaluate<void>(
    client,
    `(() => {
      const target = Array.from(document.querySelectorAll('[data-testid="store-board-v1-kpi-card"]'))
...
const main = async () => {
  const page = read("app/(workspace)/store-board/page.tsx");
  const component = read("components/store-board/v1/store-board-v1-dashboard.tsx");
  ...
  const staticChecks = {
    pageUsesV1: page.includes("StoreBoardV1Dashboard"),
    navItemsExist: navItems.every((item) => visualSources.includes(item)),
    storeBoardHighlighted: component.includes('activeLabel="店铺看板"'),
    topbarExists: component.includes('title="店铺看板"') && visualSystemSource.includes("退出"),
    controlsExist: ["目标店铺", "店铺设置", "统计时间", "自定义"].every((text) => component.includes(text)),
    noHomeTerms: !component.includes("商品排除") && !component.includes("平台目标") && !component.includes("商家备注"),
    kpisExist: kpis.every((kpi) => component.includes(kpi)),
    productTopExists: component.includes("店铺内商品贡献 TOP"),
    omittedBigModules:
      !component.includes("TOP商品") &&
      !component.includes("推广与售后") &&
      !component.includes("目标摘要"),
    noInvalidSourceText:
  ...
  const profileDir = fs.mkdtempSync(path.join(os.tmpdir(), "airburg-store-board-v1-profile-"));
  ...
  try {
    ...
    await client.send("Page.addScriptToEvaluateOnNewDocument", {
      source: `window.localStorage.setItem("airburg:demo-session", JSON.stringify({ account: "store-v1-audit", loggedInAt: "2026-06-27T00:00:00.000Z" }));`,
    });
    await client.send("Page.navigate", { url: PAGE_URL });
    await waitForSelector(client, '[data-testid="store-board-v1-dashboard"]');
    await wait(600);

    await setViewport(client, 1440, 1200);
    screenshots.push(await capture(client, "store-board-1440", "1440x1200", "base"));

    await setViewport(client, 390, 1000);
    screenshots.push(await capture(client, "store-board-390", "390x1000", "base"));

    await setViewport(client, 1440, 1200);
    await clickByText(client, "店铺设置");
    await waitForSelector(client, '[data-testid="store-board-v1-store-settings-popover"]');
    screenshots.push(await capture(client, "store-board-store-settings", "1440x1200", "store_settings_open"));
    ...
    screenshots.push(await capture(client, "store-board-return-rate-three-lines", "1440x1200", "return_rate_click"));

    await evaluate<void>(
      client,
      `document.querySelector('[data-testid="store-board-v1-product-top"]')?.scrollIntoView({ block: 'center' });`,
    );
    await wait(300);
    screenshots.push(await capture(client, "store-board-product-top", "1440x1200", "product_top"));
  } finally {
    if (client) {
  ...
  const checks = {
    ...staticChecks,
    screenshotCount: screenshots.length === 5,
  ...
  fs.writeFileSync(
    manifestPath,
    JSON.stringify({ route: "/store-board", screenshotDir, screenshots, checks, forbiddenChanged, consoleErrors: capturedConsoleErrors, ignoredConsoleErrors }, null, 2),
    "utf8",
  );
