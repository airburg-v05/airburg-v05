...
const DATABASE_NAME = "airburg-v05-d3-audit";
...
const launchChrome = async (chromePath: string, baseUrl: string): Promise<{
  ...
  const userDataDir = fs.mkdtempSync(path.join(os.tmpdir(), "airburg-v05d3-chrome-"));
...
const completionRecordIsValid = (relativePath: string, expectedTaskId: string): boolean => {
  const completion = JSON.parse(readText(relativePath)) as {
    taskId?: string;
    status?: string;
  ...
  return (
    completion.taskId === expectedTaskId &&
...
const staticDependencyChecks = (): { d1CompletionValid: boolean; d2CompletionValid: boolean } => {
  return {
    d1CompletionValid: completionRecordIsValid(D1_COMPLETION, "V0.5D_1_STORE_CONTEXT_AND_MULTI_STORE_BOARD_RELAYOUT"),
    d2CompletionValid: completionRecordIsValid(D2_COMPLETION, "V0.5D_2_STORE_BOARD_VISUAL_RUNTIME_AND_DRILLDOWN_CLOSURE"),
  };
};
...
const runRuntime = async (sensitiveValues: Set<string>): Promise<RuntimeEvidence> => {
  ...
  const screenshotDir = fs.mkdtempSync(path.join(os.tmpdir(), "airburg-v05d3-store-screenshots-"));
  ...
  try {
    ...
    await client.navigate(`${next.baseUrl}/store-board?platform=jd&storeId=tmall-default-store`);
    await client.waitForText("当前店铺不可用", 45000);
    const invalidPlatformSafe = await client.evaluate<boolean>(
    ...
    await client.navigate(`${next.baseUrl}/store-board?platform=tmall&storeId=${encodeURIComponent(secondStoreId)}`);
    await client.waitForText("当前店铺不可用", 45000);
    const nonDefaultLegacyFallbackBlocked = await client.evaluate<boolean>(
      "document.body.innerText.includes('当前店铺不可用') && !document.body.innerText.includes('旧版单店数据') && !document.body.innerText.includes('天猫默认店铺') && !document.body.innerText.includes('Error:')",
    );
    ...
    const screenshotManifest = {
      taskId: "V0.5D_3_FINAL_REGRESSION_AND_STAGE_FREEZE",
    ...
    return {
      ...
      corruptedStateSafe,
      legacyFallbackWorks,
      nonDefaultLegacyFallbackBlocked,
      emptyStateWorks,
      mobile390NoOverflow,
...
const main = async () => {
  ...
  const checks = {
    ...
    d2CompletionValid: staticChecks.d2CompletionValid,
    ...
    invalidPlatformSafe: runtime?.invalidPlatformSafe === true,
    ...
    nonDefaultLegacyFallbackBlocked: runtime?.nonDefaultLegacyFallbackBlocked === true,
  ...
  const output = {
    ...
    script: "validate-v05d-final-browser-runtime",
    databaseName: DATABASE_NAME,
    ...
    invalidPlatformSafe: runtime?.invalidPlatformSafe ?? false,
    corruptedStateSafe: runtime?.corruptedStateSafe ?? false,
    nonDefaultLegacyFallbackBlocked: runtime?.nonDefaultLegacyFallbackBlocked ?? false,
  };
