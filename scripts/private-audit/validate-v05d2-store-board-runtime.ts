...
const gitSuccess = (args: string[]): boolean => {
  try {
...
const combineDatasets = (first: V2Dataset, second: V2Dataset): V2Dataset => ({
  ...first,
  datasetId: "v05d2-audit-combined-dataset",
...
const main = async () => {
  ...
  const d1Completion = exists(D1_COMPLETION)
    ? JSON.parse(read(D1_COMPLETION)) as {
      taskId?: string;
      status?: string;
  ...
  const storeComponentSource = read("components/store-board/v05/store-board-command-center.tsx");
  const storeViewModelSource = read("lib/v05/store-board/build-view-model.ts");

  const checks: Check[] = [
    {
      name: "d1_completion_record_valid",
      pass:
        d1Completion?.taskId === "V0.5D_1_STORE_CONTEXT_AND_MULTI_STORE_BOARD_RELAYOUT" &&
        d1Completion.status === "complete" &&
        typeof d1Completion.authorizationCommit === "string" &&
        typeof d1Completion.completionCommit === "string" &&
        d1Completion.commandResults?.every((result) => result.status === "PASS") === true,
    },
    {
      name: "d1_completion_commit_is_head_ancestor",
      pass: !!d1Completion?.completionCommit && gitSuccess(["merge-base", "--is-ancestor", d1Completion.completionCommit, "HEAD"]),
    },
    {
      name: "d1_authorization_commit_is_completion_ancestor",
      pass:
        !!d1Completion?.authorizationCommit &&
        !!d1Completion?.completionCommit &&
        gitSuccess(["merge-base", "--is-ancestor", d1Completion.authorizationCommit, d1Completion.completionCommit]),
    },
    ...
    { name: "store_component_has_platform_select_label", pass: storeComponentSource.includes("store-board-platform-select") && storeComponentSource.includes("平台") },
    { name: "store_component_has_operating_focus_tabs", pass: storeComponentSource.includes("店铺运营关注") && storeComponentSource.includes("商品表现") && storeComponentSource.includes("系列进度") && storeComponentSource.includes("推广与售后") },
    { name: "store_component_has_default_product_drilldown", pass: storeComponentSource.includes("/product-board?") && storeComponentSource.includes("查看商品看板") },
    { name: "store_component_has_default_series_drilldown", pass: storeComponentSource.includes("/series-board?") && storeComponentSource.includes("查看系列看板") },
    { name: "store_component_disables_non_default_product_drilldown", pass: storeComponentSource.includes("商品看板待升级") },
    { name: "store_component_disables_non_default_series_drilldown", pass: storeComponentSource.includes("系列看板待升级") },
    { name: "store_view_model_not_changed_to_direct_storage", pass: !storeViewModelSource.includes("localStorage") && !storeViewModelSource.includes("indexedDB.open") },
    ...
    { name: "series_progress_limited", pass: defaultStore.seriesProgress.length <= 5 && secondStore.seriesProgress.length <= 5 },
    { name: "target_progress_limited", pass: defaultStore.targetProgress.length <= 4 && secondStore.targetProgress.length <= 4 },
  ...
  const output = {
    ...
    script: "validate-v05d2-store-board-runtime",
