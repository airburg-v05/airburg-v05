...
- SaaS UI V2 当前状态：`CROSS_PLATFORM_SERIES_PUBLIC_E2E_PASS_PENDING_OWNER_REVIEW`；技术 PASS 不得称为产品完成。
- Home、系列、店铺、商品、数据健康和导入历史共用当前品牌的活动运行时快照；目标中心独立读写品牌目标。
- 当前新公网版本 `visualAccepted = false`、`humanAccepted = false`；上一版已获认可的视觉基线、标签和发布目录只作为回滚依据保留。
...
- 当前任务：`SAAS_V2_CROSS_PLATFORM_SERIES_LENSES_AND_GROWTH_DRIVERS_V1`，状态 `PUBLIC_E2E_PASS_PENDING_POST_DEPLOY_OWNER_REVIEW`。
- 下一唯一入口：宗骥在其他电脑检查新公网版本，决定是否认可为新的稳定视觉基线；自动化截图不能替代用户确认。
...
- 最新已验证业务实现与公网部署源：`dface84eefdd87a55c819fd323626efb436b19b2`。
- 上一版宗骥认可的稳定版证据标签：`stable/saas-v2-commercial-refinement-20260722`，peel 到 `7f131274713608a98be35983e99b6ac2e4aa2696`；该版本继续作为回滚基线。
- 公网预览：[V2 Home](http://123.57.49.121/v2/home)。
- `/v2/home`：16 个商用显示指标形成 4×4 桌面网格；首页保持品牌范围，指标设置最多勾选 5 个系列并在同一经营指标区域追加系列摘要。
- `/v2/series-board`：粘贴商品 ID 绑定、同一套 16 指标/趋势、紧凑维护条和底部系列卡片编辑删除已公网回归；新增品牌汇总、单店拆解和渠道/店铺贡献。
...
- 新版本公网隔离系统浏览器回归为 `52/52 PASS`，console 业务错误与 failed business requests 均为 `0`。
- 新版本仍为 `visualAccepted=false`、`humanAccepted=false`；不得据此宣称完整产品、多平台接入或视觉验收已完成。
...
- TypeScript、lint、production build、专项校验及隔离浏览器回归已通过；本地和公网浏览器回归均为 `52/52 PASS`，console 业务错误和 failed business requests 均为 `0`。
- 当前不可变发布目录：`/opt/airburg/releases/saas-v2-cross-platform-series-dface84-20260722T092433`；未 push、未 merge，也未获得宗骥对新版本的视觉确认。
- 上一版发布目录 `/opt/airburg/releases/saas-v2-commercial-refinement-e25660c-20260722T003453` 与 PM2 回滚快照均保留。
...
4. 新的跨平台系列版本已公网技术 PASS，但尚未完成人工视觉和业务核查；禁止把 52/52 自动化结果写成视觉验收或业务完成。
...
7. 远端 `npm audit` 仍有 1 个 moderate 和 2 个 high 依赖告警。当前没有用户 CSS 输入或未受信任服务端图片处理路径，因此属于低当前暴露，不等于安全关闭，后续仍需兼容升级。
...
3. 当前新版本已公网技术回归；下一步仅由宗骥做视觉和业务检查，确认前不得记录 `VISUAL_ACCEPTED` 或 `HUMAN_ACCEPTED`。上一版稳定标签与发布目录继续作为回滚基线。
