...
| SaaS UI V2 home/boards | One active brand runtime through `lib/v2/home/v2-home-adapter.ts`, with explicit target overlays and browser-local series/product configuration | Data-bound; owner-approved public stable baseline plus an undeployed local candidate |
...
2. Home remains the all-platform/all-store brand cockpit. The local series candidate separates all-store brand summary from exactly-one-store execution.
...
5. The public V2 stable baseline is owner-approved. The local cross-platform candidate passed technical regression but is not deployed or visually accepted.
